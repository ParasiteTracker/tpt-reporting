...
cat datasets.tsv | xargs -L1 java -Xmx4G -jar $(which elton) review --type note >> $REVIEW

# generating interaction data
cat datasets.tsv | xargs -L1 java -Xmx4G -jar $(which elton) interactions >> $INTERACTIONS

# group review notes by collection
echo -e "#notes\tnamespace\tinstitutionCode\tcollectionCode\tnote" > $REVIEW_BY_COLLECTION
cat $REVIEW | tail -n+2 | awk -F '\t' '{ print $4 "\t" $9 "\t" $10 "\t" $6 }' | sort | uniq -c | sort -nr | sed 's/[ ]*//;s/[ ]/\t/' >> $REVIEW_BY_COLLECTION
...
cat $INTERACTIONS | tail -n+2 | awk -F '\t' '{ print $46 "\t" $5 "\t" $4 "\t" $18 "\t" $19 }' | sort | uniq -c | sort -nr | sed 's/[ ]*//;s/[ ]/\t/' >> $INTERACTIONS_BY_COLLECTION
